...
/** Parser for system-agnostic objects.
  ...
  * {{{
  *   Character Name
  *   Block 1: Block 1, paragraph 1.  Blocks are denoted by a label, followed by a colon and a space,
  *     then the start of the text.  Continuation of the text to subsequent lines is denoted by indenting the text
  ...
  *
  *
  *   Block 3: Any extra blank lines at the end of a block (like those immediately above) are not preserved in the
  *     parsed character.
  * }}}
  *
  * Multiple characters can be included in one file.  The only requirement is that characters after the first may not
  * have colons in their names (so they are not parsed as extra text blocks of the first character).
  */
object AgnosticParser extends SystemParser[Agnostic] {
  ...
  private def paragraphs: Parser[List[String]] = leadParagraph ~ repsep(paragraph, rep1(blankLine)) ^^ {
  ...
  /** The first paragraph in a text block requires special handling.
    ...
    * }}}
    */
  private def leadParagraph: Parser[Option[String]] = (opt(" " ~> ".+".r) <~ eol) ~ opt(paragraph) <~ rep(eol) ^^ {
